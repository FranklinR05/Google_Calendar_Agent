...
def list_calendar_list(max_capacity = 200):
    '''
    List calendar lists until the total number of items reachs max_capacity.

    Parameters:
    - max_capacity(int or str, optional): The maximum of calendar lists to retrieve. Defults to 200.
        if a string is provided, it will be converted to an interger. 
...
def list_calendar_events(calendar_id,max_capacity = 20):
    """
    List events from a specified calendar until the total number of events reaches max_capacity.
...
def insert_calendar_event(calendar_id, **kwargs):
    ...
    Parameters:
    - service: The Goolgle Calendar API service instance.
    -calendar_id: The ID of the calendar where the event will be inserted.
    -**kwargs": Additional keywords arguments representing the event details. 
    Returns: 
    - The created event.
    """
